...
const MovieCard = ({movie}) => {
  ...
  return (
    ...
    hover:-translate-y-1 transition duration-300 w-66'>
        ...
        src={movie.backdrop_path} alt="movie poster"  className='rounded-lg h-52 w-full 
        ...
        <div className='flex items-center justify-between mt-4 pb-3'>
            <button onClick={()=> {navigate(`/movies/${movie._id}`); window.scrollTo(0,0)}}
            className='px-4 py-2 text-xs bg-indigo-700 hover:bg-indigo-800 transition  rounded-full  font-medium cursor-pointer'>
                Buy Tickets
                </button>
