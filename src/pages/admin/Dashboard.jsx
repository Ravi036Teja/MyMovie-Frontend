...
const Dashboard = () => {
  ...
  return !loading ? (
    ...
    <BlurCircle top='-100px' left='0px'/>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full'>
        {dashboardCards.map((card,index) => (
          ...
          justify-between px-4 py-3  bg-indigo-700/10 border
          border-indigo-500/20 rounded-md max-w-50 w-full'> 
            <div>
   ...
   <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4 max-w-5xl'>
        <BlurCircle top='100px' left='-10px'/>
        {dashboardData.activeShows.map((show) => (
          <div key={show._id} className='w-55 rounded-lg overflow-hidden
          h-full pb-3 bg-indigo-700/10 border border-indigo-700/20 
